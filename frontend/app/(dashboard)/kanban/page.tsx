...
import type { Tile, Tag, KanbanColumn, KanbanFilter, Pattern, ActionType, PatternShape } from '@/types';
...
function tileMatchesFilters(tile: Tile, filters: KanbanFilter[]): boolean {
  ...
  return filters.every((f) => {
    switch (f.type) {
      ...
      case 'tag':
        return tile.tags?.some((t) => t.id === f.value);
      case 'status':
...
function getFilterLabel(f: KanbanFilter, tags: Tag[], patterns: Pattern[]): string {
  switch (f.type) {
    case 'action_type':
      return ACTION_TYPE_LABELS[f.value] || f.value;
    case 'tag': {
      const tag = tags.find((t) => t.id === f.value);
      return tag?.name || 'Tag sconosciuto';
    }
    case 'status':
      return f.value === 'completed' ? 'Completato' : 'Attivo';
    case 'pattern': {
      const pat = patterns.find((p) => p.id === f.value);
      return pat?.name || 'Pattern sconosciuto';
    }
    default:
      return f.value;
  }
...
export default function KanbanPage() {
  ...
  const allPatterns = useMemo(() => [...customPatterns], [customPatterns]);
  ...
  const updateColMutation = useMutation({
    mutationFn: (params: { id: string; updates: { title?: string; filters?: KanbanFilter[]; sort_order?: number } }) =>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex overflow-x-auto gap-3 p-4">
          {columns.map((col) => {
            const colTiles = tiles.filter((t) => tileMatchesFilters(t, col.filters));
              ...
              >
                  ...
                  <div className="flex flex-wrap gap-1 px-3 py-1.5 border-b border-zinc-800/50">
                    {col.filters.map((f, i) => (
                      ...
                      >
                        ...
                        {getFilterLabel(f, tags, patterns)}
                        <button
                          onClick={() => {
                ...
                {isFilterOpen && (
                  <div className="px-3 py-2 border-b border-zinc-800 bg-zinc-900/80 space-y-2">
                    <div className="text-[10px] text-zinc-500 font-semibold uppercase tracking-wider">Aggiungi filtro</div>

                    {!addFilterType ? (
                      <div className="flex flex-wrap gap-1">
                        {(['action_type', 'tag', 'status', 'pattern'] as const).map((type) => (
                          <button
                            key={type}
