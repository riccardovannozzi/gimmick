...
import type { Tile, Spark, PatternShape } from '@/types';
...
function generateDays(pastDays: number, futureDays: number): string[] {
  const days: string[] = [];
  const now = new Date();
...
function getMonthLabel(dayKey: string): string {
  const [y, m] = dayKey.split('-').map(Number);
  const d = new Date(y, m - 1, 1);
  return d.toLocaleDateString('it-IT', { month: 'long', year: 'numeric' });
}

function getDayOfMonth(dayKey: string): number {
...
// ─── SVG Patterns (shape-driven from Settings) ───
function ShapePattern({ shape, color }: { shape: PatternShape; color: string }) {
...
function TimelineBands({
  ...
  const moveMutation = useMutation({
    mutationFn: async ({ tileId, newDay }: { tileId: string; newDay: string }) => {
      ...
      if (!tile) return;
      const oldDate = new Date(tile.start_at || new Date());
      const [year, month, dayNum] = newDay.split('-').map(Number);
      const newStart = new Date(oldDate);
      newStart.setFullYear(year, month - 1, dayNum);
      const updates: Record<string, string | null> = { start_at: newStart.toISOString() };
      if (tile.end_at) {
        const delta = newStart.getTime() - oldDate.getTime();
        updates.end_at = new Date(new Date(tile.end_at).getTime() + delta).toISOString();
      }
      return tilesApi.update(tileId, updates as Parameters<typeof tilesApi.update>[1]);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tiles-tileview'] });
      toast.success('Tile spostato');
    },
    onError: () => toast.error('Errore spostamento'),
  });

  const handleDrop = useCallback((targetDay: string) => {
    if (!dragTileId) return;
    const tile = allTiles.find((t) => t.id === dragTileId);
    if (!tile) return;
    const currentDay = getDayKey(tile.start_at || '');
    if (currentDay !== targetDay) {
      moveMutation.mutate({ tileId: dragTileId, newDay: targetDay });
    }
    setDragTileId(null);
        setOverDay(null);
  ...
  const renderTile = (tile: Tile, type: 'event' | 'deadline', tileSize: number = TILE_FULL) => {
    const tileColor = getTagColor(tile);
    const deadlineToday = type === 'deadline' && isToday(tile.start_at || '');
    let subtitle: string | undefined;
    if (type === 'event' && tile.start_at) {
    ...
    return (
      <div
        ...
        onDragEnd={() => { setDragTileId(null); setOverDay(null); }}
        className={cn('transition-all', isDragging && 'opacity-30 scale-95')}
      >
        <TileSquare
          title={tile.title || 'Senza titolo'}
          subtitle={subtitle}
  ...
  useEffect(() => {
    if (todayRef.current && scrollRef.current) {
      ...
      container.scrollLeft = todayEl.offsetLeft - 16;
    }
  }, [days]);
            ...
            <div ref={vContainerRef} className="flex flex-col flex-1">
              ...
              <div className="flex flex-1 overflow-auto">
                ...
                <div style={{ width: `${splitPercent}%` }} className="overflow-hidden">
                  ...
                  <TodoBand tiles={todos} getTagColor={getTagColor} getTagInfo={getTagInfo} draggable selectedId={selectedTileId} onTileClick={handleTileClick} selectedTagIds={selectedTagIds} resolveShape={resolveShape} />
                </div>
                ...
                <div style={{ width: `${100 - splitPercent}%` }} className="overflow-hidden">
                  ...
                  <TodoBand tiles={notes} getTagColor={getTagColor} getTagInfo={getTagInfo} draggable selectedId={selectedTileId} onTileClick={handleTileClick} selectedTagIds={selectedTagIds} resolveShape={resolveShape} />
                </div>
              </div>
            </div>
          )}
