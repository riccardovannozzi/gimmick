...
/** CSS background-image rules for a given pattern. Apply to bg1. */
export function backgroundCSS(bgId: BackgroundId, theme: PixelTheme): React.CSSProperties {
  const c = theme.ink2, c2 = theme.ink3, accent = theme.accent;
  const op = theme.mode === 'light' ? 0.08 : 0.10;
  switch (bgId) {
    case 'none': return {};
    case 'scanlines': return {
      backgroundImage: `repeating-linear-gradient(to bottom, ${hexWithAlpha(c, op*1.4)} 0 1px, transparent 1px 3px)`,
    };
    case 'dots': return {
      backgroundImage: `radial-gradient(${hexWithAlpha(c, op*1.8)} 1.5px, transparent 1.5px)`,
      backgroundSize: '20px 20px', backgroundPosition: '10px 10px',
    };
    case 'grid': return {
      backgroundImage: `
        linear-gradient(to right, ${hexWithAlpha(c, op)} 0 1px, transparent 1px),
        linear-gradient(to bottom, ${hexWithAlpha(c, op)} 0 1px, transparent 1px)`,
      backgroundSize: '24px 24px, 24px 24px',
    };
    case 'checker': return {
      backgroundImage: `
        linear-gradient(45deg, ${hexWithAlpha(c, op*0.8)} 25%, transparent 25%, transparent 75%, ${hexWithAlpha(c, op*0.8)} 75%),
        linear-gradient(45deg, ${hexWithAlpha(c, op*0.8)} 25%, transparent 25%, transparent 75%, ${hexWithAlpha(c, op*0.8)} 75%)`,
      backgroundSize: '16px 16px',
      backgroundPosition: '0 0, 8px 8px',
    };
    case 'diagonal': return {
      backgroundImage: `repeating-linear-gradient(45deg, ${hexWithAlpha(c, op*1.4)} 0 1px, transparent 1px 8px)`,
    };
    case 'dither': return {
      backgroundImage: `
        radial-gradient(${hexWithAlpha(c, op*1.5)} 1px, transparent 1px),
        radial-gradient(${hexWithAlpha(c, op*1.5)} 1px, transparent 1px)`,
      backgroundSize: '8px 8px, 8px 8px',
      backgroundPosition: '0 0, 4px 4px',
    };
    case 'sunset': {
      const mid = hexWithAlpha(PIXEL_PALETTES[theme.paletteId][theme.mode].cap.video, theme.mode === 'light' ? 0.18 : 0.28);
      const low = hexWithAlpha(PIXEL_PALETTES[theme.paletteId][theme.mode].cap.file,  theme.mode === 'light' ? 0.20 : 0.35);
      return {
        backgroundImage: `
          repeating-linear-gradient(to bottom, transparent 0 22px, ${hexWithAlpha(c, op*1.3)} 22px 23px),
          linear-gradient(to bottom, ${theme.bg1} 0%, ${mid} 55%, ${low} 78%, ${theme.bg1} 100%)`,
      };
    }
    case 'noise': return {
      backgroundImage: `
        radial-gradient(${hexWithAlpha(c, op)} 0.5px, transparent 1px),
        radial-gradient(${hexWithAlpha(c2, op*0.7)} 0.5px, transparent 1px)`,
